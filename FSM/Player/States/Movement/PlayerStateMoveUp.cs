...
{
    internal class PlayerStateMoveUp : PlayerMovementStates
    {
        public PlayerStateMoveUp(int type, GameObject gameObject) : base(type, gameObject)
        {
        }
        ...
        public override void Enter()
        {
            if (_gameObject is GameObjects.Moving.Player.Player player)
            {
                player.BIsMovingUp = true;

                player.Direction = new Vector2(player.Direction.X, -1);

                player.Texture = AssetManager.playerTextures[1];
            }
        }
